...
Arguments = 1-1.gen_hmagma_annot.R \
          --gene $(gene) \
          --chr $(chr)
Request_memory = 1M
Request_cpus = 1
#concurrency_limits = khuang82:125
Log = ../condor_log/1-1/chr$(chr)/simple.$(gene).log
Output = ../condor_log/1-1/chr$(chr)/simple.$(gene).out
error = ../condor_log/1-1/chr$(chr)/simple.$(gene).err
